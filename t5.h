...
public:
	...
	T5_file();
	~T5_file();
    const char* getValueFromSection(const char*, const char*) const;
    ...
    //Use overlayfile as an overlay for this file. Returns the number of values that are patched.
    //Overlaid values are NOT written to file.
    int applyOverlay(const T5_file& overlayfile);
